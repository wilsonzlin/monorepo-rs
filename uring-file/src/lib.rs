...
impl Uring {
  pub async fn read_at(&self, file: &File, offset: u64, len: u64) -> Vec<u8> {
  ...
  pub async fn write_at(&self, file: &File, offset: u64, data: Vec<u8>) -> Vec<u8> {
    let (tx, rx) = oneshot::channel();
    self
      .sender
      .send(Request::Write {
  ...
  pub async fn sync(&self, file: &File) {
    let (tx, rx) = oneshot::channel();
    self
      .sender
      .send(Request::Sync {
