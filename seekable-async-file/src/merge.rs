...
mod tests {
  ...
  /// Helper to create a write request with Vec<u8> data
  fn wr(offset: u64, data: Vec<u8>) -> WriteRequest<Vec<u8>> {
    WriteRequest::new(offset, data)
  }
  ...
  fn test_merge_no_writes() {
    let writes: Vec<WriteRequest<Vec<u8>>> = vec![];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_single_write() {
    let writes = vec![wr(10, vec![1, 2, 3, 4])];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_non_overlapping_sequential() {
    let writes = vec![wr(0, vec![1, 2]), wr(2, vec![3, 4]), wr(4, vec![5, 6])];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 2, vec![1, 2]),
      (2, 4, vec![3, 4]),
  ...
  fn test_merge_non_overlapping_gaps() {
    let writes = vec![wr(0, vec![1, 2]), wr(10, vec![3, 4]), wr(20, vec![5, 6])];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 2, vec![1, 2]),
      (10, 12, vec![3, 4]),
  ...
  fn test_merge_complete_overlap_later_wins() {
    let writes = vec![
      wr(0, vec![1, 2, 3, 4]),
      wr(0, vec![5, 6, 7, 8]), // Completely overwrites first
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_partial_overlap_at_end() {
    let writes = vec![
      wr(0, vec![1, 2, 3, 4]), // [0..4)
      wr(2, vec![5, 6]),       // [2..4) overlaps
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_partial_overlap_at_start() {
    let writes = vec![
      wr(2, vec![1, 2, 3, 4]), // [2..6)
      wr(0, vec![5, 6]),       // [0..2) adjacent, not overlapping
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_write_inside_another() {
    let writes = vec![
      wr(0, vec![1, 2, 3, 4, 5, 6]), // [0..6)
      wr(2, vec![7, 8]),             // [2..4) inside
    ];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 2, vec![1, 2]),
  ...
  fn test_merge_write_spans_multiple() {
    let writes = vec![
      ...
      wr(2, vec![3, 4]),
      wr(4, vec![5, 6]),
      wr(1, vec![7, 8, 9, 10, 11]), // [1..6) spans all three
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_multiple_overlaps_later_wins() {
    let writes = vec![
      wr(0, vec![1, 2, 3, 4]),
      wr(1, vec![5, 6]), // [1..3) overlaps
      wr(2, vec![7, 8]), // [2..4) overlaps previous
    ];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 1, vec![1]),
  ...
  fn test_merge_complex_overlapping_sequence() {
    let writes = vec![
      wr(0, vec![1, 2, 3, 4, 5]), // [0..5)
      wr(3, vec![6, 7, 8, 9]),    // [3..7) extends and overlaps
      wr(1, vec![10, 11]),        // [1..3) overlaps first part
      wr(6, vec![12, 13, 14]),    // [6..9) extends further
      wr(2, vec![15]),            // [2..3) single byte
    ];
    let merged = merge_overlapping_writes(writes);
    // Expected:
    // [0..1): 1
  ...
  fn test_merge_out_of_order_offsets() {
    ...
    let writes = vec![wr(20, vec![5, 6]), wr(0, vec![1, 2]), wr(10, vec![3, 4])];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 2, vec![1, 2]),
      (10, 12, vec![3, 4]),
  ...
  fn test_merge_adjacent_not_merged() {
    ...
    let writes = vec![
      wr(0, vec![1, 2]),
      wr(2, vec![3, 4]), // Starts exactly where first ends
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_large_span_over_many_small() {
    let writes = vec![
      ...
      wr(6, vec![4]),
      wr(8, vec![5]),
      wr(1, vec![10, 11, 12, 13, 14, 15, 16, 17]), // [1..9) covers most
    ];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 1, vec![1]),
  ...
  fn test_merge_write_extends_left() {
    let writes = vec![
      wr(10, vec![1, 2, 3, 4]),
      wr(5, vec![5, 6, 7, 8, 9, 10]), // [5..11) extends left and overlaps
    ];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
  ...
  fn test_merge_write_extends_right() {
    let writes = vec![
      wr(5, vec![1, 2, 3, 4]),
      wr(7, vec![5, 6, 7, 8, 9]), // [7..12) extends right and overlaps
    ];
    let merged = merge_overlapping_writes(writes);
  ...
  fn test_merge_preserves_data_correctly() {
    ...
    let writes = vec![
      wr(0, vec![10, 20, 30, 40, 50, 60, 70, 80]), // [0..8)
      wr(2, vec![99, 88]),                         // [2..4) overwrites 30, 40
    ];
    let merged = merge_overlapping_writes(writes);
    assert_intervals(&merged, &[
      (0, 2, vec![10, 20]),
